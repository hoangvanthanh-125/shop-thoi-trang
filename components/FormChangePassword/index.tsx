...
function FormChangePassWord(props) {
  ...
  const schema = yup
    .object({
      ...
      confirmNewPass: yup.string().oneOf([yup.ref("newPass"), null], "Mật khẩu không khớp"),
    })
    .required();
  const {
    control,
  ...
  return (
    <form onSubmit={onSubmit}>
      <div className={style.wrapField}>
        <InputField type="password" placeholder="Mật khẩu cũ" name="oldPass" control={control} />
        <p className={style.notiError}>{errors.oldPass?.message}</p>
      </div>

      <div className={style.wrapField}>
        <InputField type="password" placeholder="Mật khẩu mới" name="newPass" control={control} />
      ...
      <div className={style2.action}>
        ...
        <button onClick={() => handleCancel()} className={style2.action__cancel}>
          Hủy bỏ
        </button>
      </div>
    </form>
