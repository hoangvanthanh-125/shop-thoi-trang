...
function ProductListCategory(props) {
  ...
  return (
    <div className={`${style.listCategory}`}>
      <div className={`${style.listCategory__head} theme`}>Danh mục</div>
      <div className={style.listCategory__body}>
        {listDanhmuc.map((item, index) => (
          <Link href={`/products/?type=${item.path}`} key={index}>
            <a
              className={`${style.listCategory__body__item} ${type === item.path && style.active}`}
            >
              <span>
                <ArrowRightIcon
                  className={`${style.icon} ${type === item.path && style.iconActive}`}
                />
              </span>{" "}
              <span>{item.name}</span>
            </a>
